...
#define SASSERT(condition) if ((condition)){} else {SDEBUG_LOG(SPITE_FILELINE_FUNC("FALSE\n")); SPITE_DEBUG_BREAK}
#if defined(_MSC_VER)
...
#define SASSERTM(condition,format,...) if ((condition)){} else{SDEBUG_LOG(SPITE_FILELINE_FUNC(SPITE_CONCAT(format,"\n")),__VA_ARGS__); SPITE_DEBUG_BREAK}
#endif
#endif
}
