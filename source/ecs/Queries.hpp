...
namespace spite
{
	...
	public:
		virtual void recreate(ComponentLookup* lookup,
		                      ComponentStorage* storage,
		                      const TypesVector& hasComponents,
	...
	public:
		...
		void recreate(ComponentLookup* lookup,
		              ComponentStorage* storage,
		              const TypesVector& hasComponents,
		...
		public:
			...
			iterator(const IndicesVector::iterator current,
			         const IndicesVector::iterator end,
			         ComponentTable<T>* table) : m_table(table), m_current(current), m_end(end)
			{
			}
		...
		public:
			...
			exclude_inactive_iterator(const IndicesVector::iterator current,
			                          const IndicesVector::iterator end,
			                          ComponentTable<T>* table) : m_table(table),
	...
	public:
		...
		public:
			...
			iterator(const IndicesVector::iterator current,
			         const IndicesVector::iterator end,
			         ComponentTable<T1>* table1,
			         ComponentTable<T2>* table2) : m_table1(table1), m_table2(table2),
			                                       m_current(current), m_end(end)
			{
			}
			...
			{
				eastl::tuple<T1&, T2&> tuple = eastl::make_tuple(
					m_table1->operator[](eastl::get<0>(*m_current)),
	...
	public:
		SharedQuery1(ComponentLookup* lookup,
		             ComponentStorage* storage,
		             const spite::HeapAllocator& allocator,
		             const TypesVector& hasComponents,
		             const TypesVector& hasNoComponents) : m_indices(allocator)
		...
		public:
			...
			iterator(const IndicesVector::iterator current,
			         const IndicesVector::iterator end,
			         SharedComponentTable<T>* table) : m_table(table), m_current(current),
			                                           m_end(end)
			{
			}
		...
		public:
			...
			exclude_inactive_iterator(const IndicesVector::iterator current,
			                          const IndicesVector::iterator end,
			                          SharedComponentTable<T>* table) : m_table(table),
				m_current(current), m_end(end)
			{
				// Skip inactive elements 
				while (m_current != m_end && !m_table->isActive(*m_current))
	...
	public:
		...
		{
			if (eastl::find(m_targetComponents.begin(), m_targetComponents.end(), type) !=
				m_targetComponents.end())
			{
				return true;
			}
			if (eastl::find(m_hasComponents.begin(), m_hasComponents.end(), type) != m_hasComponents
				.end())
			{
				return true;
			}
			if (eastl::find(m_hasNoComponents.begin(), m_hasNoComponents.end(), type) !=
				m_hasNoComponents.end())
			{
				return true;
			}
			return false;
	...
	class QueryBuilder
	{
		...
		eastl::hash_map<QueryBuildInfo, IQuery*, QueryBuildInfo::hash, eastl::equal_to<
			                QueryBuildInfo>, spite::HeapAllocator> m_queries;

		spite::HeapAllocator m_allocator;

	public:
		QueryBuilder(std::shared_ptr<ComponentLookup> lookup,
		             std::shared_ptr<ComponentStorage> storage,
		             const spite::HeapAllocator& allocator) : m_lookup(std::move(lookup)),
		...
		Query1<T>* buildQuery(QueryBuildInfo& buildInfo)
		{
			...
			IQuery* query = new Query1<T>(m_lookup.get(),
			                              m_storage.get(),
			                              m_allocator,
		...
		template <t_plain_component T1, t_plain_component T2>
		Query2<T1, T2>* buildQuery()
		{
			QueryBuildInfo buildInfo = getQueryBuildInfo();
			return buildQuery<T1, T2>(buildInfo);
		}
		...
		Query2<T1, T2>* buildQuery(QueryBuildInfo& buildInfo)
		{
			...
			{
				STEST_LOG_UNPRINTED(
					TESTLOG_ECS_QUERY_LOADED_FROM_CACHE((std::string(typeid(T1).name()) + std::
						string(typeid(T2).name()) ).c_str()))
				IQuery* query = m_queries.at(buildInfo);
				return static_cast<Query2<T1, T2>*>(query);
			}

			STEST_LOG_UNPRINTED(
				TESTLOG_ECS_NEW_QUERY_CREATED((std::string(typeid(T1).name()) + std::string(typeid(
					T2).name())).c_str() ))
			IQuery* query = new Query2<T1, T2>(m_lookup.get(),
			                                   m_storage.get(),
			                                   m_allocator,
			                                   buildInfo.m_hasComponents,
			                                   buildInfo.m_hasNoComponents);
		...
		Query3<T1, T2, T3>* buildQuery(QueryBuildInfo& buildInfo)
		{
			...
			if (contains(buildInfo))
			{
				...
				STEST_LOG_UNPRINTED(
					TESTLOG_ECS_QUERY_LOADED_FROM_CACHE((std::string(typeid(T1).name()) + std::
						string(typeid(T2).name()) + std::string(typeid(T3).name())).c_str()))
				return static_cast<Query3<T1, T2, T3>*>(query);
			}

			STEST_LOG_UNPRINTED(
				TESTLOG_ECS_NEW_QUERY_CREATED((std::string(typeid(T1).name()) + std::string(typeid(
					T2).name())+ std:: string(typeid(T3).name())).c_str()))
			IQuery* query = new Query3<T1, T2, T3>(m_lookup.get(),
			                                       m_storage.get(),
			                                       m_allocator,
			                                       buildInfo.m_hasComponents,
			                                       buildInfo.m_hasNoComponents);
		...
		SharedQuery1<T>* buildQuery()
		{
			...
			STEST_LOG_UNPRINTED(TESTLOG_ECS_NEW_QUERY_CREATED(typeid(T).name()))
			IQuery* query = new SharedQuery1<T>(m_lookup.get(),
			                                    m_storage.get(),
			                                    m_allocator,
			                                    buildInfo.m_hasComponents,
			                                    buildInfo.m_hasNoComponents);
		...
		//TODO implement filtering for shared components
		//template <t_shared_component T>
		...
		void recreateDependentQueries(const std::type_index type)
		{
			for (const auto& keyval : m_queries)
			{
				...
				{
					keyval.second->recreate(m_lookup.get(),
					                        m_storage.get(),
					                        keyval.first.m_hasComponents,
